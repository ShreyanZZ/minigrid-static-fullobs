...
def train_learner(env, policy, certificate, optimizer_cert,optimizer_policy, C_init,C_unsafe,C_decrease, iteration):
    ...
    for epoch in range(EPOCHS_PER_ITER):
        ...
        if len(C_init)>0:
            ...
            loss_init = torch.tensor(0.0, device=C_decrease.device)

        # L_Unsafe: Max(Barrier - V(unsafe), 0)
        if len(C_unsafe)>0:
            loss_unsafe = torch.max(torch.relu(UNSAFE_BARRIER - certificate(C_unsafe)))
        else:
            loss_unsafe = torch.tensor(0.0, device=C_decrease.device)
        ...
        if len(C_decrease)>0:
            ...
            v_next_reshaped = v_next_all.view(len(C_decrease), SAMPLES)
...
def run_verifier(env, policy, certificate, C_init, C_unsafe, C_decrease):
    ...
    if failed_init_mask.any():
        failures = C_init[failed_init_mask]
        n_sample = max(1, int(len(failures) * 0.20))
        perm = torch.randperm(len(failures))[:n_sample]
        new_samples["init"] = failures[perm]
        print(f"    -> Found {len(failures)} failures. Sampling {n_sample} for training.")
    ...
    if failed_unsafe_mask.any():
        failures = C_unsafe[failed_unsafe_mask]
        n_sample = max(1, int(len(failures) * 0.20))
        perm = torch.randperm(len(failures))[:n_sample]
        new_samples["unsafe"] = failures[perm]
        print(f"    -> Found {len(failures)} failures. Sampling {n_sample} for training.")
...
##### expectation
    ...
    with torch.no_grad():
        ...
        if len(C_decrease) == 0:
            ...
            env_noslip=make_env(ENV_NAME, seed=1,slip=0)
            next_list = []
            fake_list=[]
            for i in range(N):
                state_tensor = C_decrease[i] 
                action = intended_actions[i]
                env_noslip.load_state_from_tensor(state_tensor)
                fake_obs, _, _, _, _ = env_noslip.step(action)
                fake_list.append(fake_obs['image'])
            ...
            fake_V_next = np.stack(fake_list)
            fake_V_next = torch.tensor(fake_V_next, dtype=torch.float32, device=C_decrease.device)
            fake_V_next = certificate(fake_V_next).view(-1)
            fake_V_next = fake_V_next.view(N, 1)
            fake_V_next = fake_V_next.mean(dim=1, keepdim=True)
#####
            ...
            for i in range(4):
                print(f"sampled exp_V = {expected_V_next[i].item()} || noslip exp_V = {fake_V_next[i].item()} ")
            EPSILON_ver=1e-14
            decrease_gap = v_curr - (expected_V_next + EPSILON_ver)
            ...
            if failed_decrease_mask.any():
                # Extract failures from the relevant subset
                failures = C_decrease[failed_decrease_mask]
                n_sample = max(1, int(len(failures) * 0.20))
                perm = torch.randperm(len(failures))[:n_sample]
                new_samples["decrease"] = failures[perm]
                print(f"    -> Found {len(failures)} failures. Sampling {n_sample} for training.")
...
def main():
    ...
    # Step A: Learner (Train on current set)
        policy, certificate, history = train_learner(env,
            policy, certificate, optimizer_cert,optimizer_policy, 
            C_init,C_unsafe,C_decrease,i
        )
        # plot_learning_curve(history)

    # Step B: Verifier (Check ALL states)
        passed, new_samples = run_verifier(env, policy,certificate,all_init, all_unsafe, all_decrease )
        if new_samples["init"] is not None:
            C_init = torch.cat([C_init, new_samples["init"]])
            C_init = torch.unique(C_init, dim=0)
        if new_samples["unsafe"] is not None:
            C_unsafe = torch.cat([C_unsafe, new_samples["unsafe"]])
            C_unsafe = torch.unique(C_unsafe, dim=0)

        if new_samples["decrease"] is not None:
            C_decrease = torch.cat([C_decrease, new_samples["decrease"]])
            C_decrease = torch.unique(C_decrease, dim=0)

        print(f"Updated Training Sets | Init: {len(C_init)}, Unsafe: {len(C_unsafe)}, Decrease: {len(C_decrease)}")
