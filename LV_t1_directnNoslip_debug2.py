...
def train_learner(env, policy, certificate, optimizer_cert,optimizer_policy, C_init,C_unsafe,C_decrease, iteration):
    """
    ...
    for epoch in range(EPOCHS_PER_ITER):
        ...
        if len(C_init)>0:
            ...
            loss_init = torch.tensor(0.0, device=C_unsafe.device)

        # L_Unsafe: Max(Barrier - V(unsafe), 0)
        if len(C_unsafe)>0:
            loss_unsafe = torch.max(torch.relu(UNSAFE_BARRIER - certificate(C_unsafe)))
        else:
            loss_unsafe = torch.tensor(0.0, device=C_unsafe.device)
        ...
        if len(C_decrease)>0:
            ...
            V_curr = certificate(C_decrease).view(-1)
            ...
            intended_actions = intended_actions.cpu().numpy()
            next_list=[]
            all_neighbors_list = [] 
            neighbor_counts = []
            ...
            for i in range(len(C_decrease)):
                ...
                env.load_state_from_tensor(state_tensor)
                curr_pos = base_env.agent_pos
                valid_neighbors = []
                # Check 4 directions: Up, Down, Left, Right
                for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    nx, ny = curr_pos[0] + dx, curr_pos[1] + dy
                    if 0 <= nx < base_env.width and 0 <= ny < base_env.height:
                        cell = base_env.grid.get(nx, ny)
                        # Only consider valid walkable neighbors (not walls)
                        # Adjust this condition if slipping into a wall is possible/terminal
                        if cell is None or (cell.type != 'wall'): 
                            # Move agent explicitly to neighbor and observe
                            base_env.agent_pos = (nx, ny)
                            obs_neigh = base_env.gen_obs()
                            obs_neigh=env.observation(obs_neigh)
                            valid_neighbors.append(obs_neigh['image'])
                all_neighbors_list.extend(valid_neighbors)
                neighbor_counts.append(len(valid_neighbors))

            huge_batch_np = np.stack(next_list)
            huge_batch = torch.tensor(huge_batch_np, dtype=torch.float32, device=C_decrease.device)
            V_next = certificate(huge_batch).view(-1)
            batch_neighbors = torch.tensor(np.stack(all_neighbors_list), dtype=torch.float32, device=C_decrease.device)
            V_neighbors_all = certificate(batch_neighbors).view(-1)
            V_neighbors_avg_list = []
            cursor = 0
            for count in neighbor_counts:
                # Slice the segment belonging to state i
                v_segment = V_neighbors_all[cursor : cursor + count]
                V_neighbors_avg_list.append(v_segment.mean())
                cursor += count
            V_neighbors_avg = torch.stack(V_neighbors_avg_list)
            DELTA=0.03
            weighted_V_next = (1.0 - DELTA) * V_next + DELTA * V_neighbors_avg
            ...
            violation = weighted_V_next + EPSILON - V_curr
            loss_expected = torch.mean(torch.relu(violation))
            loss_policy = torch.mean(torch.relu(violation).detach() * action_log_probs * 100)
        else:
            loss_expected = torch.tensor(0.0, device=C_decrease.device)
            loss_policy = torch.tensor(0.0,device=C_decrease.device)
        ...
        loss = (loss_init) + (loss_unsafe) + 100*(loss_expected)
        ...
        p_grad = pol_w.grad.abs().mean().item() if pol_w.grad is not None else 0.0
        print(f"       > Policy Weights: Mean={p_mean:.5f} | Grad={p_grad:.5f} ")
        ...
        if epoch % 200 == 0:
            ...
            checkpoint = {
                ...
                "optimizer_state": optimizer_cert.state_dict(),
...
##### expectation
    ...
    with torch.no_grad():
        v_curr = certificate(C_decrease).view(-1, 1)
        expected_V_next=[]
        if len(C_decrease) == 0:
            ...
            next_list = []
            all_neighbors_list = [] # Term B: Neighbors (Slip)
            neighbor_counts = []
            base_noslip = env.unwrapped
            ...
            for i in range(N):
                ...
                env.load_state_from_tensor(state_tensor)
                curr_pos = base_noslip.agent_pos

                valid_neighbors = []
                # Check Up, Down, Left, Right
                for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    nx, ny = curr_pos[0] + dx, curr_pos[1] + dy
                    if 0 <= nx < base_noslip.width and 0 <= ny < base_noslip.height:
                        cell = base_noslip.grid.get(nx, ny)
                        # Only consider walkable cells (not walls)
                        if cell is None or (cell.type != 'wall'):
                            base_noslip.agent_pos = (nx, ny)
                            obs_neigh = base_noslip.gen_obs()
                            obs_neigh=env.observation(obs_neigh)
                            valid_neighbors.append(obs_neigh['image'])
            ...
            huge_batch_np = np.stack(next_list)
            huge_batch = torch.tensor(huge_batch_np, dtype=torch.float32, device=C_decrease.device)
            v_next_intended = certificate(huge_batch).view(-1)
            batch_neighbors = torch.tensor(np.stack(all_neighbors_list), dtype=torch.float32, device=C_decrease.device)
            v_neighbors_all = certificate(batch_neighbors).view(-1)
            v_neighbors_avg_list = []
            cursor = 0
            for count in neighbor_counts:
                ...
                cursor += count
            v_neighbors_avg = torch.stack(v_neighbors_avg_list)
            weighted_V_next = (1.0 - DELTA) * v_next_intended + DELTA * v_neighbors_avg
            weighted_V_next = weighted_V_next.view(N, 1)


            EPSILON_ver=1e-14
            decrease_gap = v_curr - (weighted_V_next + EPSILON_ver)
            ...
            print(f"[3] Decrease Condition (V_next + {EPSILON_ver} <= V_curr):")
...
MODEL_PATH = "storage/randomenv_ranreward_16_directnNoslip_dbg2/status.pt" 
CERTI_PATH = "certis/16x16_2Lyrs.pt"
...
EPOCHS_PER_ITER = 4000 # How long to train per loop

def main():
    # 1. Setup Universe (All possible states)
    env = make_env(ENV_NAME, seed=1,slip=0)
    ...
    # optimizer_cert.load_state_dict(checkpoint["optimizer_state"])
    ...
    policy, certificate, history = train_learner(env,
        policy, certificate, optimizer_cert,optimizer_policy,  C_init, C_unsafe, C_decrease, 1
    )
